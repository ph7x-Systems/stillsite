...
class DbApiBackend(StorageBackend):
    ...
    def list_form_submissions(
        ...
        where = f" WHERE {' AND '.join(clauses)}" if clauses else ""
        rows = self._fetchall(
            # Literal fragments only; values are bound parameters. The
            # row cap is applied client-side for engine portability.
    ...
    def delete_form_submission(self, submission_id: str) -> bool:
        with self._tx():
            cursor = self._execute("DELETE FROM form_submissions WHERE id = %s", (submission_id,))
            count = int(cursor.rowcount)
        return count > 0
    ...
    def prune_form_submissions(self, before: datetime) -> int:
        with self._tx():
            cursor = self._execute(
                "DELETE FROM form_submissions WHERE received_at < %s", (before.isoformat(),)
            )
            count = int(cursor.rowcount)
        return count
    ...
    def list_preview_links(self, entry_kind: str, entry_id: str) -> list[PreviewLink]:
        rows = self._fetchall(
            "SELECT id, entry_kind, entry_id, created_at, expires_at, revoked"
            " FROM preview_links WHERE entry_kind = %s AND entry_id = %s"
    ...
    def revoke_preview_link(self, link_id: str) -> bool:
        with self._tx():
            cursor = self._execute(
                "UPDATE preview_links SET revoked = 1 WHERE id = %s AND revoked = 0",
                (link_id,),
            )
            count = int(cursor.rowcount)
        return count > 0
    ...
    def get_or_create_secret(self, name: str, factory: Callable[[], str]) -> str:
        row = self._fetchone("SELECT value FROM app_secrets WHERE name = %s", (name,))
        if row is not None:
            return str(row[0])
        value = factory()
        try:
            with self._tx():
                self._execute(
                    "INSERT INTO app_secrets (name, value) VALUES (%s, %s)", (name, value)
                )
        except Exception:
